...
def main():
    ...
    try:
        print(f"\n[{model_name}] 모델을 불러오는 중입니다 (최초 실행 시 인터넷 연결 및 모델 다운로드가 진행됩니다)...")
    ...
    if not os.path.exists(audio_file):
        print(f"[오류] '{audio_file}' 파일을 찾을 수 없습니다. 경로와 이름을 확인해 주세요.")
        return

    try:
        print(f"\n'{audio_file}' 파일 변환을 시작합니다...")
        base_name, _ = os.path.splitext(audio_file)
        output_txt = f"{base_name}_result.txt"
        
        # 변환 실행 (실시간 로그 출력)
        result = model.transcribe(audio_file, language='Korean', verbose=True)

        # 결과 저장
        with open(output_txt, 'w', encoding='utf-8') as f:
            f.write(result['text'])

        print(f'\n[성공] 변환 완료! 결과가 [{output_txt}] 파일로 저장되었습니다.')
